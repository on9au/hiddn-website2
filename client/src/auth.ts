...
export const isUserAuth = async (
    ...
    // Check api endpoint '/api/auth/is-logged-in' to see if user is authenticated
    try {
        const result = await fetch("/api/auth/is-logged-in");
        ...
        if (result.status === 200) {
            ...
            expiration.setMinutes(
                expiration.getMinutes() + cache_valid_minutes
            );
            localStorage.setItem(
                "isAuthenticated",
                JSON.stringify({ isAuthenticated: true, expiration })
            );
            return;
        }
    } catch (error) {
...
export const isUserAdmin = async (
    ...
    try {
        ...
        if (result.status === 200) {
            ...
            localStorage.setItem(
                "isAdmin",
                JSON.stringify({ isAdmin: true, expiration })
            );
            return;
        }
    } catch (error) {
